...
Public Class QuickStartForm
    ...
    Private Const HelpViewerArguments As String = "/helpcol ms-help://ms.EntLib.2006Jan /LaunchFKeywordTopic LoggingQS1"
    ...
    Private Sub DisplayConfiguration()
        Try
            ...
            For Each source As TraceSourceData In sources
                ...
                If Not (Logger.GetFilter(Of CategoryFilter)().ShouldLog(source.Name)) Then
                    results.Append("*")
                End If
    ...
    Private Sub checkLogginButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles checkLogginButton.Click
        ...
        If result = Windows.Forms.DialogResult.OK Then
            Try
                ...
                ' ----------------------------------------------------------------------
                If Logger.GetFilter(Of LogEnabledFilter)().Enabled Then
                    ' Logging is enabled.
                ...
                ' user would pass the filter check.
                ' ----------------------------------------------------------------------
                If Logger.GetFilter(Of PriorityFilter)().ShouldLog(priority) Then
                ...
                ' ----------------------------------------------------------------------
                ' Query the Logger class to determine if an event with the
                ...
                Else
                    If Not Logger.GetFilter(Of LogEnabledFilter)().Enabled Then
